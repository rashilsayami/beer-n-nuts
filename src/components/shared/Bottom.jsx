...
const Bottom = () => {
  ...
  const commonButtonClasses = (path) => `
    ...
    ${isActive(path) ? "text-[#f5f5f5] bg-[#343434]" : "text-[#ababab] hover:bg-[#343434] hover:text-[#f5f5f5]"}
  `;
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#262626] px-4 py-2 h-16 flex items-center justify-between">
      ...
      <button 
        disabled={isActive("/tables") || isActive("/menu") || isActive("/more") || isActive("/inventory") || isActive("/report")}
        onClick={openModal}
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center justify-between bg-[#1f1f1f] px-6 py-4 rounded-lg">
              ...
              <span className="text-white text-lg font-medium">{guestCount} {guestCount === 1 ? 'person' : 'people'}</span>
              <button 
                onClick={increment} 
                className="text-[#f6B100] text-2xl hover:text-[#e5a400] transition-colors w-8 h-8 flex items-center justify-center"
              >
